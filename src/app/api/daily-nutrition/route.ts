...
export async function GET(request: NextRequest) {
  try {
    ...
    if (date) {
      query = query.where(eq(dailyNutrition.date, date));
    }

    if (metricId) {
      query = query.where(eq(dailyNutrition.metricId, parseInt(metricId)));
    }

    // Add proper ordering by date
    query = query.orderBy(asc(dailyNutrition.date));
